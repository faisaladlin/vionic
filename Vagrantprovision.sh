...
# Default provisioning configurations ...
...
# Boolean flag to install (1) or skip (0) services installatiom ...
...
if [ ${SET_HOST_NAME} != ubuntu-bionic ]; then
	...
	if [[ ${SET_HOST_NAME} =~ ^[a-z0-9\-]+$ ]]; then
		...
		sed -i -e 's/ubuntu-bionic/'${SET_HOST_NAME}'/g' /etc/hostname
    	sed -i -e 's/ubuntu-bionic/'${SET_HOST_NAME}'/g' /etc/hosts

	else

		echo $'\n------------------------------------------------------------------'
		echo INVALID Hostname: ${SET_HOST_NAME} \| Defaulted to ubuntu-bionic

		SET_HOST_NAME=ubuntu-bionic
	fi
fi

if [ ${SETUP_MONGODB} = 1 ] || [ ${SETUP_REDIS} = 1 ] || [ ${SETUP_PHP7FPM} = 1 ] || [ ${SETUP_PHP5FPM} = 1 ] || [ ${SETUP_BEANSTALKD} = 1 ] || [ ${SETUP_SUPERVISOR} = 1 ] || [ ${SETUP_WEBMIN} = 1 ]; then
	...
	if [ ${SETUP_REDIS} = 1 ] || [ ${SETUP_PHP7FPM} = 1 ] || [ ${SETUP_PHP5FPM} = 1 ] || [ ${SETUP_BEANSTALKD} = 1 ] || [ ${SETUP_SUPERVISOR} = 1 ]; then
...
if [ ${SETUP_BUILD} = 1 ]; then
	...
	[[ ${SETUP_NODE8} = 1 ]] && BUILD_PACKAGES="${BUILD_PACKAGES} nasm"
...
if [ ${SETUP_MYSQL} = 1 ]; then
    ...
    # listen to all client (do not apply in production)
...
if [ ${SETUP_NGINX} = 1 ]; then
	...
	sed -i -e 's|# deny access to .htaccess files|# deny access to dot-prefixed \& vagrant files\n\tlocation ~ (/\\..+\|/Vagrantfile\|/Vagrantprovision\\.sh) {\n\t\tdeny all;\n\t}\n\n\t# deny access to .htaccess files|g' /etc/nginx/sites-available/default
fi
...
if [ ${SETUP_PHP7FPM} = 1 ] || [ ${SETUP_PHP5FPM} = 1 ]; then
	...
	if [ ${SETUP_PHP7FPM} = 1 ]; then
		...
		if [ ${SETUP_APACHE} = 1 ]; then
			...
			sed -i -e 's|index index.html index.htm|index index.html index.htm index.php|g' /etc/nginx/sites-available/default
			sed -i -e 's|#location ~ \\.php|location ~ \\.php$ {\n\t\tinclude snippets/fastcgi-php.conf;\n\t\tfastcgi_pass unix:/var/run/php/php7.1-fpm.sock;\n\t}\n\t#location ~ \\.php|g' /etc/nginx/sites-available/default
		...
		if [ -f /vagrant/artisan ]; then
			...
			for fileordirname in `ls -A /vagrant`; do
				if [ ${VAGRANT_EMPTY} = 1 ]; then
					if [ ${fileordirname} != ".DS_Store" ] && [ ${fileordirname} != "thumbs.db" ] && [ ${fileordirname} != "desktop.ini" ] && [ ${fileordirname} != ".git" ] && [ ${fileordirname} != ".gitignore" ] && [ ${fileordirname} != ".gitattributes" ] && [ ${fileordirname} != ".idea" ] && [ ${fileordirname} != ".vagrant" ] && [ ${fileordirname} != "Vagrantfile" ] && [ ${fileordirname} != "Vagrantprovision.sh" ] && [ ${fileordirname} != "readme.md" ]; then
						...
						# Consider vagrant folder NOT EMPTY if found file/directory OTHER THAN:
						# .DS_Store, thumbs.db, desktop.ini, .git, .gitignore, .gitattributes,
						# .idea, .vagrant, Vagrantfile or Vagrantprovision.sh
			...
			if [ ${VAGRANT_EMPTY} = 0 ]; then
				...
				if [ ${SETUP_LARAVEL} = 1 ]; then
					if [ ${SETUP_PHP7FPM} = 1 ]; then
						composer create-project --prefer-dist laravel/laravel project "5.6.*"
					else # SETUP_PHP5FPM
						composer create-project --prefer-dist laravel/laravel project "5.4.*"
					fi
				elif [ ${SETUP_LUMEN} = 1 ]; then
					if [ ${SETUP_PHP7FPM} = 1 ]; then
						composer create-project --prefer-dist laravel/lumen project "5.6.*"
					else # SETUP_PHP5FPM
